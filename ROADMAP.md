...
**Status (2026-07-16): implemented.** Browser PNG import uses a CORE encoder
that matches the Python encoder byte-for-byte on a shared synthetic golden
vector; Playwright covers tile/sprite replacement and consecutive multi-frame
append. Gun and engine layers round-trip through full SPRITE records. `.rapmod`
v2 and `mod2glb` validate and apply position-addressed replaced/appended PIC
items while retaining v1 compatibility and emitting v1 when no art is embedded.
MIDI format 0/1 conversion covers tempo maps, running status,
percussion, controllers and pitch bend at Raptor's 70 Hz MUS clock; format 2
and SMPTE timing are rejected explicitly. A synthetic format-0 golden output
is pinned byte-for-byte to midi3mus 1.0.0 with its Raptor-specific `-t 70`
setting. All paths
use synthetic, redistributable fixtures.
...
the same input (a shared synthetic RGBA/indexed/encoded golden vector, plus
browser-generated PNG fixtures — no game data), plus decode(encode(x))
round-trips in `test_editor_core.mjs`. Browser-only PNG decoding stays outside
CORE; width, height, RGBA bytes, and the palette feed DOM-free CORE
...
Add a `"pics"` array to `.rapmod`. Replacements carry archive number, item
index, original name, base hash, and base64 PIC data; the positional identity
is required because shipped archives contain unnamed and duplicate-name art.
Appends carry a unique name and target archive. Import strictly validates both
the new data and the replacement target before applying. Use version 2 only
when this section is present; v1 files keep importing.
...
14-bit → 8-bit pitch bend), rescale delta times to Raptor's 70 Hz MUS clock,
...
events spot-checked), plus a golden file pinning output against midi3mus 1.0.0
at Raptor's 70 Hz rate.
